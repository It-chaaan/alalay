...
export const updateWalletSchema = z.object({
  ...
  interest_crediting_frequency: z.enum(["monthly", "quarterly", "yearly", "other"]).nullable().optional(),
}).strict();
...
export const walletTransferSchema = z.object({
  ...
  if (value.from_wallet_id === value.to_wallet_id) ctx.addIssue({ code: 'custom', path: ['to_wallet_id'], message: 'Choose a different destination wallet.' });
});

export const walletInterestSchema = z.object({
  amount: currencyAmount.refine((value) => value > 0, "Interest amount must be greater than zero."),
  date: safeDate,
  note: z.string().trim().max(500).nullable().optional(),
}).strict();
