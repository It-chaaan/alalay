...
export type WalletTransferPayload = { from_wallet_id: string; to_wallet_id: string; amount: number; fee: number; transfer_method?: "instapay" | "pesonet" | "internal" | "other" | null; date: string; note?: string | null; idempotency_key: string };
...
export async function getWallet(userId: string, id: string) {
  ...
  const [income, expenses, bills, adjustments, transfers] = await Promise.all([
    ...
    client().from("wallet_transfers").select("id, from_wallet_id, to_wallet_id, amount, fee, transfer_method, note, transferred_at, created_at").eq("user_id", ownerId).or(`from_wallet_id.eq.${id},to_wallet_id.eq.${id}`).order("transferred_at", { ascending: false }),
  ]);
  ...
  const transactions = [
    ...
    ...(transfers.data ?? []).map((row) => { const counterpartId = row.from_wallet_id === id ? row.to_wallet_id : row.from_wallet_id; const counterpartName = counterpartNames.get(counterpartId) ?? "wallet"; const fee = asNumber(row.fee); return { ...row, kind: row.from_wallet_id === id ? "transfer_out" : "transfer_in", label: row.from_wallet_id === id ? `Transfer to ${counterpartName}${fee ? ` · Fee ₱${fee.toLocaleString("en-PH")}` : ""}` : `Transfer from ${counterpartName}`, date: row.transferred_at, amount: row.from_wallet_id === id ? -asNumber(row.amount) - fee : asNumber(row.amount) }; }),
...
export async function createWalletInterest(userId: string, walletId: string, payload: { amount: number; date: string; note?: string | null }) {
  const ownerId = requireUserId(userId);
  const { data: wallet, error: walletError } = await client().from("wallets").select("id, name").eq("id", walletId).eq("user_id", ownerId).single();
  if (walletError || !wallet) throw new AppError(404, "not_found", "Wallet not found.");
  const { data, error } = await client().from("income").insert({ user_id: ownerId, wallet_id: walletId, source: payload.note?.trim() || `${wallet.name} interest`, type: "interest", amount: payload.amount, date: payload.date, is_recurring: false }).select("id, wallet_id, source, type, amount, date").single();
  throwIfError(error);
  return data;
}
