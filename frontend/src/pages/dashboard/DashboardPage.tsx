...
function AiInsightCard({ insight }: { insight: DashboardSummary["ai_insight"] }) {
  ...
  return (
    <article className="flex h-full w-full max-w-[680px] flex-col rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-800">{insight.message}</p>
        </div>
      </div>
      <div className="mt-auto grid gap-3 pt-5">
        <div className="rounded-xl bg-brand-muted p-3 text-brand-dark">
    ...
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => moveMonth(-1)} aria-label="Previous month" className="grid h-7 w-7 place-items-center rounded-lg text-slate-500 hover:bg-slate-100"><ChevronLeft className="h-4 w-4" /></button>
          <span className="min-w-28 text-center text-xs font-semibold text-slate-700">{monthTitle(visibleMonth)}</span>
          <button type="button" onClick={() => moveMonth(1)} aria-label="Next month" className="grid h-7 w-7 place-items-center rounded-lg text-slate-500 hover:bg-slate-100"><ChevronRight className="h-4 w-4" /></button>
        </div>
      </div>
      ...
      <div className="mt-1 grid grid-cols-7 gap-0.5">
        {cells.map((date, index) => {
          ...
          return (
            <button key={dateKey} type="button" onClick={() => dayItems.length && setSelectedDate(selected ? null : dateKey)} className={`min-h-10 rounded-lg px-0.5 py-0.5 text-left transition ${dayItems.length ? "hover:bg-brand-soft" : "cursor-default"} ${selected ? "bg-brand-soft ring-1 ring-brand-primary" : ""}`} aria-label={`${date.toDateString()}${dayItems.length ? `, ${dayItems.length} due` : ""}`}>
              <span className={`mx-auto grid h-5 w-5 place-items-center rounded-full text-[11px] ${dateKey === todayKey ? "bg-brand-primary font-bold text-white ring-2 ring-brand-primary/20" : "text-slate-700"}`}>{date.getDate()}</span>
              {dayItems.length ? <span className="mt-0.5 flex justify-center gap-0.5"><span className="h-1.5 w-1.5 rounded-full bg-amber-400" />{dayItems.some((item) => item.kind === "subscription") ? <span className="h-1.5 w-1.5 rounded-full bg-violet-500" /> : null}</span> : <span className="mt-1 block h-1.5" />}
              <span title={dayItems[0]?.name} className="mt-0.5 block truncate text-center text-[8px] leading-3 text-slate-500">{dayItems[0]?.name ?? ""}</span>
              {dayItems.length > 1 ? <span className="block truncate text-center text-[8px] font-semibold leading-3 text-brand-primary">+{dayItems.length - 1} more</span> : <span className="block h-3" />}
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-2 flex gap-3 text-[10px] text-slate-500"><span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-amber-400" />Bill</span><span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-violet-500" />Subscription</span></div>
    </article>
  );
}

function UpcomingWidget({ bills, subscriptions, onViewCalendar }: { bills: Bill[]; subscriptions: Subscription[]; onViewCalendar: () => void }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={onViewCalendar} className="text-xs font-semibold text-brand-primary hover:text-brand-dark">View full calendar</button>
      </div>
      <div className="mt-3 space-y-1">
        {visibleItems.length ? visibleItems.map((item) => (
          <div key={item.id} className="flex min-w-0 items-center gap-2 rounded-lg px-1 py-2">
            <span className={`h-2 w-2 shrink-0 rounded-full ${item.kind === "bill" ? "bg-amber-400" : "bg-violet-500"}`} aria-hidden="true" />
            ...
            <span className="shrink-0 font-mono text-xs font-semibold text-slate-950">{formatCurrency(item.amount)}</span>
          </div>
...
function CalendarModal({ bills, subscriptions, onClose }: { bills: Bill[]; subscriptions: Subscription[]; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-label="Full due dates calendar">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 className="font-semibold text-slate-950">Full calendar</h2><button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100" aria-label="Close full calendar"><X className="h-4 w-4" /></button></div>
        <div className="p-4"><DueDatesCalendar bills={bills} subscriptions={subscriptions} /></div>
      </div>
    </div>
  );
}
        ...
        {upcomingBills.length ? (
          ...
          <div key={bill.id}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 flex items-center gap-1 text-xs text-slate-500"><CalendarDays className="h-3.5 w-3.5" aria-hidden="true" />{formatBillListDate(bill.due_date)}</p>
            </div>
            ...
            <button type="button" onClick={() => void handlePay(bill)} disabled={payingId === bill.id} className="shrink-0 rounded-full bg-brand-primary px-3 py-2 text-xs font-semibold text-white transition hover:bg-brand-dark disabled:cursor-wait disabled:opacity-60">{payingId === bill.id ? "Paying…" : "Pay Now"}</button>
            </div>
          </div>
          );
            })}
          </>
        ) : <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No upcoming unpaid bills.</p>}
      </div>
    </article>
  );
}
