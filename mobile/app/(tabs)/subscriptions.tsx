...
export default function Subscriptions() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError('');
    try {
      const [subs, entries, walletRows] = await Promise.all([authenticatedApiRequest<Sub[]>('/api/subscriptions'), authenticatedApiRequest<Income[]>('/api/income'), fetchWallets()]);
      setRows(subs);
      setIncome(entries);
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={styles.header}><Pressable accessibilityLabel="Back to dashboard" onPress={() => router.back()} style={styles.back}><ArrowLeft size={21} color={formPalette.ink} /></Pressable><View style={styles.titleWrap}><Text style={styles.eyebrow}>RECURRING PAYMENTS</Text><Text style={styles.title}>Subscriptions</Text></View></View>
    {loading ? <View style={styles.center}><ActivityIndicator color={formPalette.accent} /><Text style={styles.muted}>Loading subscriptions…</Text></View> : error ? <View style={styles.card}><Text style={styles.cardTitle}>Subscriptions unavailable</Text><Text style={styles.muted}>{error}</Text><Pressable onPress={() => void refresh()} style={styles.retry}><Text style={styles.retryText}>Try again</Text></Pressable></View> : <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <FinancialOverviewCard eyebrow="SUBSCRIPTIONS" primaryLabel="MONTHLY SUBSCRIPTIONS" value={`₱${Math.round(monthlyTotal).toLocaleString('en-PH')}/mo`} supportingText={rows.length ? `₱${Math.round(yearly).toLocaleString('en-PH')}/year · ${percent === null ? 'Income unavailable' : `${percent.toFixed(1)}% of income`}` : 'No active subscriptions'} />
    ...
    {open && <SubscriptionForm wallets={wallets} onClose={() => setOpen(false)} onSaved={async () => { setOpen(false); await refresh(); }} />}
  </SafeAreaView>;
}

function SubscriptionForm({ wallets, onClose, onSaved }: { wallets: Wallet[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      void category;
      await authenticatedApiRequest('/api/subscriptions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim(), amount: value, renewal_date: date, billing_cycle: cycle, auto_renew: reminder, wallet_id: walletId }) });
      await onSaved();
    } catch (e) {
