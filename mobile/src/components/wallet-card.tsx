...
export function WalletCard({
  ...
  const metadata = `${accountLabel} · PHP`;
  const amountLabel = wallet.account_type === 'credit' ? 'OUTSTANDING' : 'BALANCE';

  return (
    <Pressable
      accessibilityRole="button"
      accessibilityLabel={`${wallet.name}, ${metadata}, ${amountLabel.toLowerCase()} ${amountsVisible ? balance : 'hidden'}`}
      onPress={onPress}
      style={({ pressed }) => [
        styles.card,
        { backgroundColor: wallet.color, borderColor: `${wallet.color}CC` },
    ...
    >
      ...
      <View
        pointerEvents="none"
        style={[styles.highlight, { backgroundColor: colors.textOnPrimary }]}
      />
      <View style={styles.content}>
        ...
        <View style={styles.balanceArea}>
          <Text style={styles.balanceLabel}>{amountLabel}</Text>
          <Text
