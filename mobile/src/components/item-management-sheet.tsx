...
export function ItemManagementSheet({ visible, title, itemName, deleteDescription, onClose, onEdit, onDelete, deleting = false, error = '' }: ItemManagementSheetProps) {
  ...
  const themed = { sheet: { backgroundColor: colors.surfaceElevated }, text: { color: colors.textPrimary }, secondary: { color: colors.textSecondary }, input: { backgroundColor: colors.surfaceInput }, border: { borderColor: colors.border }, dangerSurface: { backgroundColor: colors.danger + '22' } };
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}><View style={styles.overlay}><Pressable style={[styles.backdrop, { backgroundColor: colors.overlay }]} onPress={onClose} /><View style={[confirming ? styles.confirmSheet : styles.sheet, themed.sheet]}>{confirming ? <><View style={styles.titleRow}><View><Text style={styles.eyebrow}>CONFIRM DELETE</Text><Text style={[styles.title, themed.text]}>Delete {itemName}?</Text></View><Pressable accessibilityLabel="Close" onPress={onClose} style={[styles.close, themed.input]}><X size={20} color={colors.textPrimary} /></Pressable></View><Text style={[styles.description, themed.secondary]}>{deleteDescription}</Text>{error ? <Text style={[styles.error, { color: colors.danger }]}>{error}</Text> : null}<View style={styles.confirmActions}><Pressable disabled={deleting} onPress={() => setConfirming(false)} style={styles.cancelButton}><Text style={[styles.cancelText, themed.secondary]}>Cancel</Text></Pressable><Pressable disabled={deleting} onPress={() => void onDelete()} style={[styles.deleteButton, { backgroundColor: colors.danger }, deleting && styles.disabled]}>{deleting ? <ActivityIndicator color={colors.textOnPrimary} /> : <Text style={[styles.deleteText, { color: colors.textOnPrimary }]}>Delete</Text>}</Pressable></View></> : <><View style={styles.titleRow}><View><Text style={styles.eyebrow}>{title.toUpperCase()}</Text><Text style={[styles.title, themed.text]}>{itemName}</Text></View><Pressable accessibilityLabel="Close" onPress={onClose} style={[styles.close, themed.input]}><X size={20} color={colors.textPrimary} /></Pressable></View><Pressable accessibilityRole="button" onPress={() => { onClose(); onEdit(); }} style={[styles.action, themed.input]}><Pencil size={18} color={colors.textPrimary} /><Text style={[styles.actionText, themed.text]}>Edit {title.toLowerCase()}</Text></Pressable><Pressable accessibilityRole="button" onPress={() => setConfirming(true)} style={[styles.deleteAction, themed.dangerSurface]}><Trash2 size={18} color={colors.danger} /><Text style={[styles.deleteActionText, { color: colors.danger }]}>Delete {title.toLowerCase()}</Text></Pressable><Pressable accessibilityRole="button" onPress={onClose} style={styles.cancelAction}><Text style={[styles.cancelText, themed.secondary]}>Cancel</Text></Pressable></>}</View></View></Modal>;
}
