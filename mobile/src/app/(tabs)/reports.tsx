...
import { CalendarDays, CheckCircle2, Lightbulb, TrendingDown, TrendingUp, WalletCards } from 'lucide-react-native';
...
function manilaDateValue(date: Date) { const parts = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Manila', year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(date); return `${parts.find((part) => part.type === 'year')?.value}-${parts.find((part) => part.type === 'month')?.value}-${parts.find((part) => part.type === 'day')?.value}`; }
...
function IncomeVsSpending({ report }: { report: Report }) {
  ...
  const shortLabel = (value: string) => value.length > 7 ? value.slice(5) : value;
...
function makeStyles(themePalette: typeof palette) { const p = themePalette; return StyleSheet.create({
  ...
  filterContainer: { padding: 4, borderRadius: 16, backgroundColor: p.surface, borderWidth: 1, borderColor: p.line }, periods: { gap: 4 }, periodChip: { minHeight: 35, flex: 1, paddingHorizontal: 11, alignItems: 'center', justifyContent: 'center', borderRadius: 12 }, periodChipActive: { backgroundColor: p.accentPale }, periodText: { color: p.muted, fontSize: 11, fontWeight: '800' }, periodTextActive: { color: p.ink, fontWeight: '900' },
  ...
  card: { padding: 14, borderRadius: 18, backgroundColor: p.surface, borderWidth: 1, borderColor: p.line }, summaryCard: { padding: 16, borderRadius: 20, backgroundColor: p.ink }, cardEyebrow: { color: p.muted, fontSize: 10, fontWeight: '900', letterSpacing: 1, textTransform: 'uppercase' }, cardTitle: { color: p.ink, fontSize: 16, fontWeight: '900' }, cardSubtitle: { marginTop: 3, color: p.muted, fontSize: 11, fontWeight: '600' }, cardHint: { color: p.muted, fontSize: 10, fontWeight: '700' }, sectionHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, summaryTitle: { marginTop: 4, color: '#FFFFFF', fontSize: 19, fontWeight: '900' }, summaryMetrics: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 18 }, metricLabel: { color: 'rgba(255,255,255,0.66)', fontSize: 10, fontWeight: '700' }, summaryIncome: { marginTop: 3, color: '#7DE0B4', fontSize: 16, fontWeight: '900' }, summaryExpense: { marginTop: 3, color: '#FFB09B', fontSize: 16, fontWeight: '900' }, summaryNet: { marginTop: 3, color: '#FFFFFF', fontSize: 16, fontWeight: '900' }, warning: { color: p.warning },
  chartSummary: { flexDirection: 'row', alignItems: 'flex-end', gap: 22, marginTop: 14 }, chartMetricLabel: { color: p.muted, fontSize: 10, fontWeight: '700' }, chartIncome: { marginTop: 2, color: p.savings, fontSize: 15, fontWeight: '900' }, chartExpense: { marginTop: 2, color: p.expense, fontSize: 15, fontWeight: '900' }, chartTooltip: { flex: 1, alignItems: 'flex-end', padding: 7, borderRadius: 10, backgroundColor: p.expensePale }, tooltipTitle: { color: p.ink, fontSize: 10, fontWeight: '900' }, tooltipText: { marginTop: 2, color: p.muted, fontSize: 9, fontWeight: '700' }, groupedChart: { height: 126, flexDirection: 'row', alignItems: 'flex-end', gap: 7, marginTop: 14, paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: p.line }, group: { flex: 1, minWidth: 28, alignItems: 'center', justifyContent: 'flex-end', height: '100%', paddingHorizontal: 2, borderRadius: 9 }, groupSelected: { backgroundColor: p.accentPale }, groupBars: { width: '100%', height: '84%', flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center', gap: 3 }, incomeBar: { width: 8, minHeight: 3, borderRadius: 5, backgroundColor: p.savings }, expenseBar: { width: 8, minHeight: 3, borderRadius: 5, backgroundColor: p.expense }, groupLabel: { maxWidth: 52, marginTop: 5, color: p.muted, fontSize: 9, fontWeight: '700' }, chartLegend: { flexDirection: 'row', gap: 16, marginTop: 10 }, legendItem: { flexDirection: 'row', alignItems: 'center', gap: 5 }, legendDot: { width: 7, height: 7, borderRadius: 4 }, legendText: { color: p.muted, fontSize: 10, fontWeight: '700' },
