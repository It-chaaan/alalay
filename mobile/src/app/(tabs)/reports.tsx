...
function Donut({ categories }: { categories: Category[] }) {
  ...
  return <View style={styles.donutWrap}><SegmentedRing radius={42} trackColor={palette.accentPale} segments={categories.map((category) => ({ key: category.name, value: category.amount, color: getCategoryMeta(category.name).color }))}><View style={styles.donutCenter}><Text style={styles.donutPercent}>{top ? `${Math.round(top.percent)}%` : '0%'}</Text><Text style={styles.donutLabel}>{top?.name ?? 'No spend'}</Text></View></SegmentedRing></View>;
}
...
function makeStyles(themePalette: typeof palette) { const palette = themePalette; return StyleSheet.create({
  ...
  filterContainer: { padding: 6, borderRadius: 18, backgroundColor: palette.surface, borderWidth: 1, borderColor: palette.line }, periods: { gap: 6 },
  periodChip: { minHeight: 36, paddingHorizontal: 12, borderRadius: 13, justifyContent: 'center' }, periodChipActive: { backgroundColor: palette.accentPale }, periodText: { color: palette.muted, fontSize: 12, fontWeight: '800' }, periodTextActive: { color: palette.ink },
