...
import { ArrowDownUp, ChevronRight, MoreHorizontal as MoreHorizontalIcon, Plus, WalletCards, X as CloseIcon } from 'lucide-react-native';
...
function WalletDetailSheet({ detail, wallets = [], onClose, onDelete, onDeposited }: { detail: WalletDetail; wallets?: Wallet[]; onClose: () => void; onDelete: () => Promise<void>; onDeposited: () => Promise<void> }) {
  ...
  return <Modal visible animationType="slide" transparent onRequestClose={onClose}>
    <View style={styles.detailOverlay}><View style={styles.detailSheet}>
      ...
      <View style={styles.detailMoneyActions}><Pressable accessibilityRole="button" onPress={() => setDepositOpen(true)} style={[styles.depositButton, styles.moneyAction]}><Plus size={18} color="#FFFFFF" /><Text style={styles.depositText}>Deposit</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={`Transfer funds from ${wallet.name}`} onPress={() => setTransferOpen(true)} style={[styles.transferDetailButton, styles.moneyAction]}><ArrowDownUp size={17} color={colors.primary} /><Text style={styles.transferDetailText}>Transfer</Text></Pressable></View>
      ...
      {optionsOpen && <RecordActionSheet visible title="Wallet options" recordName={wallet.name} onClose={() => setOptionsOpen(false)} actions={[{ label: 'Delete', tone: 'destructive', confirm: { title: `Delete ${wallet.name}?`, message: `${wallet.name} will be removed. Income records linked to it will be moved to Cash. Other linked transactions will keep their financial history.` }, onPress: onDelete }]} />}
...
function TransferForm({ wallets, initialFrom, onClose, onSaved }: { wallets: Wallet[]; initialFrom?: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    if (source && value > Number(source.balance)) { setError(`${source.name} doesn't have enough available funds.`); return; }
    setSaving(true); setError('');
    try { await authenticatedApiRequest('/api/wallets/transfers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ from_wallet_id: fromId, to_wallet_id: toId, amount: value, date, note: note.trim() || null, idempotency_key: requestKey.current }) }); notifyFinancialMutation(); await onSaved(); }
    catch (e) { setError(e instanceof Error ? e.message : 'Unable to transfer funds right now. Please try again.'); }
