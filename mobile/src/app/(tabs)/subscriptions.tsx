...
export default function Subscriptions() {
  ...
  const toggle = async (row: Sub) => { try { await authenticatedApiRequest(`/api/subscriptions/${row.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ auto_renew: !row.auto_renew }) }); notifyFinancialMutation(); await refresh(); } catch (e) { Alert.alert('Could not update', e instanceof Error ? e.message : 'Try again.'); } };
  const remove = async (row: Sub) => { setDeleting(true); setDeleteError(''); try { await authenticatedApiRequest(`/api/subscriptions/${row.id}`, { method: 'DELETE' }); notifyFinancialMutation(); setManaging(null); await refresh(); } catch (e) { setDeleteError(e instanceof Error ? e.message : 'Subscription could not be deleted.'); throw e; } finally { setDeleting(false); } };
