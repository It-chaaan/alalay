...
import { Pencil, Plus, WalletCards, X } from 'lucide-react-native';
...
function BudgetForm({ month, summary, onClose, onSaved }: { month: string; summary: Summary | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const options = useMemo(() => entries.map((entry) => ({ label: entry.name, icon: categoryIcon(entry.name) })), [entries]);
  ...
  const addCategory = (label: string) => {
    ...
    setEntries((current) => [...current, { id, name: label, budget: 0 }]);
  ...
  return <FinanceFormSheet title={summary ? 'Edit budget' : 'Create budget'} eyebrow={monthLabel(month).toUpperCase()} amount={amount} onAmountChange={changeAmount} error={error} saving={saving} saveLabel={summary ? 'Save budget' : 'Create budget'} onSave={() => void save()} onClose={onClose}>
    ...
    <CategoryChipRow label="Budget category" value={selectedLabel} onChange={(label) => setSelectedId(entries.find((entry) => entry.name === label)?.id ?? selectedId)} options={options} showAllOptions onAdd={availableCategories.length ? () => setAddCategoryOpen(true) : undefined} />
    <Modal visible={addCategoryOpen} transparent animationType="slide" onRequestClose={() => setAddCategoryOpen(false)}><Pressable style={styles.addCategoryBackdrop} onPress={() => setAddCategoryOpen(false)}><Pressable style={styles.addCategorySheet} onPress={(event) => event.stopPropagation()}><View style={styles.addCategoryHeader}><Text style={styles.addCategoryTitle}>Add budget category</Text><Pressable accessibilityRole="button" accessibilityLabel="Close add category" onPress={() => setAddCategoryOpen(false)}><X size={20} color={formPalette.ink} /></Pressable></View>{availableCategories.length ? <View style={styles.addCategoryList}>{availableCategories.map(({ label, icon: Icon }) => <Pressable key={label} accessibilityRole="button" onPress={() => addCategory(label)} style={({ pressed }) => [styles.addCategoryOption, pressed && styles.pressed]}><Icon size={18} color={formPalette.accent} /><Text style={styles.addCategoryText}>{label}</Text><Plus size={16} color={formPalette.accent} /></Pressable>)}</View> : <Text style={styles.muted}>All available budget categories are already included.</Text>}</Pressable></Pressable></Modal>
    ...
    <Text style={styles.formHint}>Monthly Goal allocations are planned amounts only; they do not reduce income, wallet balances, or expenses.</Text>
    <Text style={styles.formHint}>Monthly Goal allocations are planned amounts only; they do not reduce income, wallet balances, or expenses.</Text>
  </FinanceFormSheet>;
}
