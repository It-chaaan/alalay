...
import { Pencil, PiggyBank, Plus, WalletCards, X } from 'lucide-react-native';
...
const preferenceOptions = [
  { value: 'auto_general', label: 'General/unallocated' },
...
function BudgetForm({ month, summary, onClose, onSaved }: { month: string; summary: Summary | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const options = useMemo(() => [...entries.map((entry) => ({ label: entry.name, icon: categoryIcon(entry.name) })), savingsBudgetOption], [entries]);
  ...
  return <FinanceFormSheet title={summary ? 'Edit budget' : 'Create budget'} eyebrow={monthLabel(month).toUpperCase()} amount={amount} onAmountChange={changeAmount} error={error} saving={saving} saveLabel={summary ? 'Save budget' : 'Create budget'} onSave={() => void save()} onClose={onClose}>
    ...
    <CategoryChipRow label="Budget category" value={isSavings ? savingsBudgetOption.label : selectedLabel} onChange={(label) => { if (label === savingsBudgetOption.label) setSelectedId('savings'); else setSelectedId(entries.find((entry) => entry.name === label)?.id ?? selectedId); }} options={options} showAllOptions onAdd={availableCategories.length ? () => setAddCategoryOpen(true) : undefined} />
